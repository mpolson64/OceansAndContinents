...
const mergeWithFirstEqualZero = (first, second) => {
    ...
    const onlyFirst = first.filter(d => !secondSet.has(d.key)).map(d => ({ key: d.key, value: 0 }));
    return d3.merge([second, onlyFirst]).sort((a, b) => d3.ascending(a.key, b.key));
};

const update = (data, filteredData) => {
    const counts = coalesceHistogram(
        histogram(data, filteredData, document.getElementById('donutSelect').value, x => x),
    );
