...
// initialize table
const table = new Tabulator('#tableChart', {
    height: '500px',
    layout: 'fitColumns',
    pagination: 'local',
    paginationSize: 20,
...
const filtersChanged = () => {
    // filter data
    const reducedPredicate = Object.values(predicates).reduce((composed, predicate) => item =>
        composed(item) && predicate(item),
    );
    ...
    document
        .getElementById('downloadFilterStateBtn')
        .setAttribute('href', `data:text/plain;charset=utf-8,${jsyaml.dump(getFiltersState())}`);
};
...
occupationFilter.onchange = () => {
    predicates.occupation = obj => obj.Occupation.toLowerCase().includes(occupationFilter.value.toLowerCase());
...
masterResidenceFilter.onchange = () => {
    predicates.masterResidence = obj =>
        obj['Master Residence'].toLowerCase().includes(masterResidenceFilter.value.toLowerCase());
    filtersChanged();
};

const registrationDistrictFilter = document.getElementById('registrationDistrictFilter');
registrationDistrictFilter.onchange = () => {
    predicates.registrationDistrict = obj =>
        obj['Registration District'].toLowerCase().includes(registrationDistrictFilter.value.toLowerCase());
    filtersChanged();
};
...
d3.csv('boc.csv').then(rawData => {
    ...
    document.getElementById('downloadFilteredBtn').onclick = () =>
        table.download('csv', 'OceansAndContinentsFiltered.csv');
    document
        .getElementById('downloadFilterStateBtn')
        .setAttribute('href', `data:text/plain;charset=utf-8,${jsyaml.dump(getFiltersState())}`);
});
